...
const Product = (props) => {
        ...
        const handleQuantity = (type) => { if (type === "inc")
        ...
        const updateStock = (payload) => { if (props.product && (cart.size.id || payload.id_size) && (cart.color.id || payload.id_color)) 
            { setStock( props.product.stock.filter( (value) => (value.sizeId === cart.size.id || value.sizeId === payload.id_size)
                && (value.colorId === cart.color.id || value.colorId === payload.id_color)).map((value) => value.available_quantity)[0]);
            }
        };
